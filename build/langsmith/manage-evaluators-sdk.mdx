...
{/* TODO(DOC-1271): Replace the version placeholders with the confirmed minimum langsmith versions from Sineha Manivannan. PY_MIN = minimum PyPI version, JS_MIN = minimum npm version. */}
- Python: `langsmith>=PY_MIN` (PyPI)
- TypeScript: `langsmith>=JS_MIN` (npm)

<Note>
In Python, the online evaluator methods are available in the synchronous client only. Async support is not available.
</Note>

The examples below initialize the client with no arguments, which reads the `LANGSMITH_API_KEY` and `LANGSMITH_ENDPOINT` environment variables. Configure your API key through environment variables rather than hardcoding it.
