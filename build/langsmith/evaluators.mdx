...
Use the LangSmith SDK to create evaluators programmatically. Evaluators created through the SDK appear in the **Evaluators** table alongside those created in the UI.
...
<Note>
Managing evaluators through the SDK requires `langsmith>=PY_MIN` (Python, PyPI) or `langsmith>=JS_MIN` (TypeScript, npm). In Python, these methods are available in the synchronous client only.
</Note>
