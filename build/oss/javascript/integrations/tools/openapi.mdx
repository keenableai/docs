...
This will help you getting started with `OpenApiToolkit` [OpenAPI toolkit](/oss/javascript/integrations/tools/openapi). `OpenApiToolkit` now lives in `@langchain/classic`. For migration details, see the [LangChain v1 migration guide](/oss/javascript/migrate/langchain-v1#langchainclassic).
